...
export function scanForUnloadedRanges(_ref3) {
  ...
  var isMultiple = function isMultiple(range) {
  ...
  if (rangeStopIndex !== null) {
    ...
    var _index = range.stopIndex + 1;
    while (
      _index < rowCount &&
      !isMultiple(range) &&
      !isRowLoaded({index: _index})
    ) {
      range.stopIndex = _index++;
    }
  ...
  if (unloadedRanges.length) {
    var _range = unloadedRanges[0];

    var _index2 = _range.startIndex - 1;
    while (
      _index2 >= 0 &&
      !isMultiple(_range) &&
      !isRowLoaded({index: _index2})
    ) {
      _range.startIndex = _index2--;
    }
  }
